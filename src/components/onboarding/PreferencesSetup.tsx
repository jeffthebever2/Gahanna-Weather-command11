...
export function PreferencesSetup({ onNext, onBack }: { onNext: () => void; onBack: () => void }) {
  ...
  const handleSave = () => {
    localStorage.setItem('user_preferences', JSON.stringify({ units, features }));
    onNext();
  };
  ...
  return (
    <div className="py-8">
      ...
      <div className="mb-8">
        ...
        <RadioGroup value={units} onValueChange={setUnits}>
          <div className="flex items-center space-x-2 p-4 rounded-lg bg-white/5 border border-white/10 mb-2">
            <RadioGroupItem value="imperial" id="imperial" checked={units === 'imperial'} />
          ...
          <div className="flex items-center space-x-2 p-4 rounded-lg bg-white/5 border border-white/10">
            <RadioGroupItem value="metric" id="metric" checked={units === 'metric'} />
        ...
        <div className="space-y-3">
          <div className="flex items-center space-x-3 p-4 rounded-lg bg-white/5 border border-white/10">
            <Checkbox
              ...
              onChange={(event) =>
                setFeatures({ ...features, severe: event.target.checked })
              }
            />
          ...
          <div className="flex items-center space-x-3 p-4 rounded-lg bg-white/5 border border-white/10">
            <Checkbox
              ...
              onChange={(event) => setFeatures({ ...features, snow: event.target.checked })}
            />
          ...
          <div className="flex items-center space-x-3 p-4 rounded-lg bg-white/5 border border-white/10">
            <Checkbox
              ...
              onChange={(event) => setFeatures({ ...features, marine: event.target.checked })}
            />
          ...
          <div className="flex items-center space-x-3 p-4 rounded-lg bg-white/5 border border-white/10">
            <Checkbox
              ...
              onChange={(event) => setFeatures({ ...features, aviation: event.target.checked })}
            />
      ...
      <div className="flex justify-between">
        <Button type="button" onClick={onBack} className="border border-white/20 text-white">
          Back
        </Button>
